...
- Normative checklist: **`organizational_memory/AGENT-FILE-STANDARD.md`**
...
- JSON Schema: **`factory/schemas/agent-registry.schema.json`**.

## Phase 3 — Output schemas + validation

- **`factory/schemas/pm-output.schema.json`** — extended task shape (compatible with minimal **`factory/task-queue.json`** rows).
- **`factory/schemas/dev-output.schema.json`**, **`factory/schemas/quality-output.schema.json`**
...
- Existing **`npm run parallel-plan`** + **`factory/task-graph.ts`** (`computeParallelBatches`).
...
- Today: **`templates/vertical-saas-spec.template.md`** + **`npm run app:configure`** / **`app:scaffold`**.

## Phase 11 — Organizational memory

- **`organizational_memory/`** — architecture, process, QMS, lean. Agents `@` these paths per **`factory/context-packs`**.
