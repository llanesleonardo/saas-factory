...
- **`how_to_run`** (object)
  ...
  - For `doc_procedure`: `{ "doc_path": "organizational_memory/<...>.md" }`
...
- **`scope`** (string): which area the tool is for (`factory/`, `organizational_memory/`, `apps/<vertical>-instance/`, etc.).
...
```json
{
  ...
  "artifacts": [
    { "type": "artifact", "path": "parallel-plan.json", "description": "parallel plan output (uploaded as CI artifact)" }
  ],
  "scope": "factory/"
}
```
