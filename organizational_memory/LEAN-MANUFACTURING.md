...
- The form **App / project bucket** writes `app:core-saas` | `app:dentist-instance` | `app:plumber-instance` | `app:mission-control-instance` into the body; **Route lean issue to app label** adds that label so the issue can land on the right **GitHub Project** (next section).
...
- Workflows: **`issue-add-to-app-project`** adds the issue to the project URL in the variable that matches its `app:*` label; **`issue-route-lean-app-label`** adds the `app:*` label from Lean waste body text.
