...
## Context
`factory/task-queue.json` is the canonical inventory for planning (`factory:next`, `parallel-plan`) and for human orchestration. Historically, validation was partial, leaving room for silent drift (bad statuses, missing fields, unknown dependencies, inconsistent conventions).
...
## Decision
- Introduce a first-class JSON Schema: `factory/schemas/task-queue.schema.json`.
