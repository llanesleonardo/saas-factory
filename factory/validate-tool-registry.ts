...
import agentRegistry from "./agent-registry.json" with { type: "json" };
...
function readRegistry(repoRoot: string): ToolRegistry {
  const p = path.join(repoRoot, "factory", "tool-registry.json");
  const raw = readFileSync(p, "utf8");
  const parsed = JSON.parse(raw) as unknown;
  if (!isRecord(parsed)) throw new Error("factory/tool-registry.json must be an object");
  if (parsed.schema_version !== 1) throw new Error("tool-registry schema_version must be 1");
  if (!Array.isArray(parsed.tools)) throw new Error("tool-registry tools[] must be an array");
  // basic normalization: ensure entries have required keys before casting
  const tools = (parsed.tools as unknown[]).map((t, i) => {
