...
import { ensureTelemetryDir, repoRootFromHere } from "./telemetry.js";
...
function suggestionLibrary(pkt: FailurePacket): {
  ...
  if (text.includes("validate-task-queue") || text.includes("task queue")) {
    ...
    fixPlan.push("Run `npm run validate-task-queue` to reproduce and read the failing task id(s).");
    fixPlan.push("If per-app queues are stale, run `npm run task-queues:sync` and re-run validation.");
    rerun.add("npm run validate-task-queue");
    rerun.add("npm run task-queues:sync");
    patchScope.add("factory/task-queue.json");
    patchScope.add("factory/task-queues/*.json");
    handoff = "tooling";
  }
  ...
  if (text.includes("validate-tool-registry") || text.includes("tool-registry")) {
    ...
    fixPlan.push("Run `npm run validate-tool-registry` and update `factory/tool-registry.json` references.");
    rerun.add("npm run validate-tool-registry");
    patchScope.add("factory/tool-registry.json");
    handoff = "tooling";
  }
...
async function main(): Promise<void> {
  ...
  if (argv.length === 0 || hasFlag(argv, "--help") || hasFlag(argv, "-h")) {
    console.log(
      [
        "Usage:",
        "  npm run factory:self-heal -- --quality <path-to-quality-output.json> [--out <path.md>] [--json]",
        "",
        "Notes:",
        "- Generates a strictly-gated self-healing report (no auto-merge, no code changes).",
        "- Output defaults under factory/.local/self-heal/ (gitignored).",
      ].join("\n"),
    );
    process.exitCode = 0;
    return;
