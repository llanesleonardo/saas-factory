...
> **Implementation note:** In-repo **`factory_internal_ops/cost.ts`**, **`hosting-cost-shared.ts`**, and **`mfg line cost` / `mfg line hosting-cost`** have been **removed** for now. This document remains the **target contract** for cost events, rollups, and FinOps evidence when you reintroduce tooling (CLI, spreadsheet, or cloud billing export).
...
## 3) Definitions
- **Run**: a single invocation of a factory command (e.g. `factory:next`, `mfg line orchestrate`, `factory:deploy`) or a role-driven activity that produces evidence.
- **App**: the owner bucket from `task.app` (e.g. `apps/todo-instance`, `factory/`, `factory/06_knowledge_base/`).
...
The factory **may** later expose a baseline hosting estimator CLI again; until then, record estimates manually (spreadsheet or vendor calculator) and, when you adopt this schema, append **`CostEvent`** lines with `source: "estimated"` and clear **`notes`**.

When using any baseline estimate to create cost events:
- `kind: "infra"`
- `source: "estimated"`
