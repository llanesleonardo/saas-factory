...
- Contract source: `factory/factory_cli/line-next.ts` + `factory/planner.ts`
...
### 7.2.1 Alternate queue files (`--queue=`)
The canonical work inventory is **`factory/03_assembly_lines/03-registry/registry/task-queue.json`** (see loader default in `factory/factory_libs/planning/task-graph.ts`).

- **Optional path**: `mfg line next`, `line queue`, `line done`, and `line orchestrate` accept **`--queue=<path>`** to read a different JSON file (same `Task[]` / `{ tasks }` shape), e.g. validator fixtures or a scratch copy for review.
- There is **no** automated per-app slice generator; use one canonical queue (with **`app`** on tasks for reporting) unless you maintain extra JSON files by hand.

**Acceptance requirements:**
- Any queue file passed to **`--queue=`** **SHALL** pass **`npm run mfg -- validate task-queue -- --queue=<same-path>`**.
...
- Contract source: `factory/planner.ts`, invoked by `factory/factory_cli/line-next.ts`
...
- Output validation schemas live in `factory/factory_schemas/*` and are validated via `npm run validate-agent-output`.
...
- **Deterministic planning** (next task suggestion, WIP cap): `factory/factory_cli/line-next.ts`, `factory/planner.ts`
- **Runbook printing orchestrator**: `factory/orchestrator.ts`
- **Schema-backed outputs** for key handoffs: `factory/factory_schemas/*` + `npm run validate-agent-output`
...
- Spec for contract + examples: `factory/06_knowledge_base/factory_specs/factory-os-tool-registry-spec.md`
...
- Spec for environment model + gate policy: `factory/06_knowledge_base/factory_specs/factory-os-deploy-engine-spec.md`
...
- Spec for event model + dashboard views: `factory/06_knowledge_base/factory_specs/factory-os-telemetry-spec.md`
...
- Spec for event model + rollups + provenance rules: `factory/06_knowledge_base/factory_specs/factory-os-cost-tracking-spec.md`
...
- Spec for inputs/actions/invariants: `factory/06_knowledge_base/factory_specs/factory-os-self-healing-spec.md`

---

## 11) Out of scope (explicit exclusions)
- **Auto-running agents in-process** (or unattended) as part of MVP orchestration.
- **Replacing vertical instance folders with config-only runtime** (explicitly rejected by `factory/06_knowledge_base/architecture/ARCHITECTURE.md`).
...
- **Planning determinism**
  - For the same queue + WIP cap, `mfg line next` / `line-next` **SHALL** suggest the same task deterministically (priority then id).
...
- **Role router + invocation model**: `factory/06_knowledge_base/routing/AGENTS.md`
- **Vertical architecture constraints (apps/packages boundaries)**: `factory/06_knowledge_base/architecture/ARCHITECTURE.md`
...
- **WIP + next-task selection contract**: `factory/factory_cli/line-next.ts`, `factory/planner.ts`
- **Orchestration constraint (runbook printing, not auto-execution)**: `factory/orchestrator.ts`
- **Output validation schemas**: `factory/factory_schemas/*`
