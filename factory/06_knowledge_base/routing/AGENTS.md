...
There is **one** Cursor chat. You choose a **role** by `@`-mentioning the matching file and telling the model to follow it for this turn. **Shortcut:** run **slash commands** under **`.cursor/commands/agent-*.md`** (e.g. `agent-dev`) which point at the same prompts. **Registry:** **`factory/02_workforce/02_00_agents/agent-registry.json`** (categories, `next_agents`, schemas).

Handoff overview: see **`factory/06_knowledge_base/agents_docs/AGENT-MAP.md`**.

**Lean manufacturing (humans + agents):** see **`factory/06_knowledge_base/process/LEAN-MANUFACTURING.md`** — value stream, WIP limits, quality at source, waste checklist, kaizen. Ask Cursor to apply it with `@factory/06_knowledge_base/process/LEAN-MANUFACTURING.md` (e.g. "we have too much WIP—suggest concrete repo/process changes").
...
|------------|----------------|---------------------|
| Turn a spec into a **task list** (JSON, no code) | `agents/pm-agent.md` | "Act as PM Agent per this file. Output only the JSON task list for `configs/apps/todo/specs/todo-spec.md`." |
...
| **Generate / expand** a full vertical **spec** | `agents/spec-generator-agent.md` | With `configs/apps/<vertical>/specs/_generated/<vertical>-SPEC-PROMPT.md` after `npm run generate-spec`, or "expand section 6 of `configs/apps/todo/specs/todo-spec.md`." |
| **Improve** the **spec markdown** | `agents/spec-generator-agent.md` | "Act as Spec Generator Agent. Update `configs/apps/todo/specs/todo-spec.md` with: …" |
...
| **Deploy, rollback, CI/Vercel runbooks** | `agents/devops-agent.md` | "Act as DevOps Agent. Document rollback for todo-instance preview deploy." |
| **README, runbooks, operator docs** | `agents/docs-agent.md` | "Act as Docs Agent. Add a 'First run' section to README for todo-instance." |
| **QMS — controlled docs & lessons** | `agents/docs-agent.md` + `factory/06_knowledge_base/qms_docs/inbox/*.md` | "Act as Docs Agent. Turn these inbox records into a `published/` controlled document + update `LESSONS-LEARNED.md` per `agents/docs-agent.md`." |
...
| **Plans, Stripe, metering, unit economics, instance profitability** | `agents/finops-agent.md` | "Act as FinOps Agent. Estimate MRR vs hosting + Stripe COGS for todo-instance; recommend optimize vs sunset review tasks." |
...
| **Bootstrap a new vertical app** (`apps/<id>-instance/`, configs, wiring) | `agents/builder-agent.md` | "Act as Builder Agent: scaffold `electrician-instance` from todo-instance pattern; no auto clone pipeline yet." |
| **Lean / flow / waste / WIP / kaizen** | `factory/06_knowledge_base/process/LEAN-MANUFACTURING.md` | "Using LEAN-MANUFACTURING.md, review our process for: …" (normative doc; pair with Tooling/PM for changes.) Track waste in **GitHub Issues** (optional `lean issue` label); use **`app:*`** labels + **`GITHUB-PROJECTS-SETUP.md`** for project boards. |

You can **@ more than one file** (e.g. `@configs/apps/todo/specs/todo-spec.md` + `@agents/pm-agent.md`).
...
- **PM**: owns what becomes a task id in `factory/03_assembly_lines/03-registry/registry/task-queue.json` (scope, acceptance criteria, dependencies, priority, closure expectations).
...
- **Docs**: owns operator docs and QMS consolidation (promoting `factory/06_knowledge_base/qms_docs/inbox/` into `factory/06_knowledge_base/qms_docs/published/`, updating `LESSONS-LEARNED.md`, maintaining cross-links).
...
- **QMS inbox records (`factory/06_knowledge_base/qms_docs/inbox/`)**: each role writes its own record after substantive work (per `agents/agent-record-for-qms.md`)
- **Published QMS docs (`factory/06_knowledge_base/qms_docs/published/`)**: Docs Agent only (curation + document control)
...
Standalone **verification / validation** procedures live under **`factory/06_knowledge_base/qms_docs/published/`** (registry **`qms_docs/DOCUMENT-CONTROL.md`**, overview **`qms_docs/README.md`**): **QMS-PUB-001** system validation strategy · **QMS-PUB-002** system verification (acceptance) · **QMS-PUB-003** subsystem verification (acceptance) · **QMS-PUB-004** unit & device test plan. **`factory/02_workforce/02_00_agents/agent-registry.json`** → **`references.qms_ivv_procedures`** lists the same paths for tooling.
...
Use the exact filename: `pm-agent`, `builder-agent`, `dev-agent`, `quality-agent`, `fix-agent`, `git-agent`, `spec-generator-agent`, `architect-agent`, `security-agent`, `devops-agent`, `docs-agent`, `support-agent`, `tooling-agent`, `finops-agent`, `spike-agent`. (**`testing-agent`** / **`qa-agent`** remain as short redirects to **`quality-agent`**.) For lean practices (not a code role), use **`@factory/06_knowledge_base/process/LEAN-MANUFACTURING.md`**.
...
- "Break `configs/apps/todo/specs/todo-spec.md` into tasks under 2 hours each, with `depends_on`."
...
**Output:** JSON task list → paste into `factory/03_assembly_lines/03-registry/registry/task-queue.json`.
...
- "Create `apps/electrician-instance/` mirroring `todo-instance` + `vercel.json` + stub `index.html`; add `configs/electrician.json`."
...
**Output:** checklist + minimal diffs; **no** claim of a fully automated clone/apply pipeline (see **`factory/06_knowledge_base/architecture/ARCHITECTURE.md`** Builder section).
...
- "Implement `TODO-005` in `apps/todo-instance` only; branch `feature/TODO-005`."
...
- "Define a manual test checklist for the todo app from the spec."
...
- "Turn `configs/apps/todo/specs/_generated/todo-SPEC-PROMPT.md` into a full `configs/apps/todo/specs/todo-spec.md`."
- "Add a 'Data retention' subsection under compliance for HIPAA."
- "Mark Phase 2 items clearly; tighten MVP acceptance bullets."

**Output:** updates under `configs/apps/<vertical>/specs/<vertical>-spec.md` — blueprint-style spec (entities + relationships + lifecycles, deterministic workflows, integration map, structured NFRs); **no** task JSON or code here (then **PM** for backlog).
...
**Call when:** onboarding, operator guides, README, API usage docs for humans — and when you need **QMS-inspired** outputs: **controlled procedures** under **`factory/06_knowledge_base/qms_docs/published/`**, **document control** metadata, **Mermaid** process diagrams, and the rolling **`factory/06_knowledge_base/qms_docs/LESSONS-LEARNED.md`** register from **`factory/06_knowledge_base/qms_docs/inbox/`** records (see **`agents/agent-record-for-qms.md`**).
...
**Output:** markdown updates; **published** docs use **`factory/06_knowledge_base/qms_docs/TEMPLATE-CONTROLLED-DOCUMENT.md`**; no invented flags without verifying repo; do **not** claim ISO certification unless the org truly has it.
...
**Example use cases** — "Map 'per location' pricing to Stripe Prices." · "Health JSON for todo-instance given exports." · "Rules when margin drops below 40%."
...
| **QMS evidence → procedures / lessons** | Raw **`factory/06_knowledge_base/qms_docs/inbox/`** per role, then **`@agents/docs-agent.md`** to curate **`published/`** + **`LESSONS-LEARNED.md`** |
...
### Task queue file (optional `--queue=`)

By default the factory reads **`factory/03_assembly_lines/03-registry/registry/task-queue.json`**. To point planners or the orchestrator at another JSON file (e.g. a fixture), pass **`--queue=<path>`**:
- **List / next / done**: `npm run mfg -- line queue` · `line next` · `line done` (each supports `--queue=`).
- **Parallel waves for a specific queue**: load that queue JSON and call `computeParallelBatches` from `factory/factory_libs/planning/task-graph.ts` (optional `--queue=` path in a one-off script); there is no dedicated `mfg line parallel` CLI.
- **Runbook for a specific queue file**: `npm run mfg -- line orchestrate -- --queue=<path-to-task-queue.json>`
