...
**Decision (locked for this repo):** verticals ship as **separate deployable apps** under **`apps/<vertical>/`** — each vertical gets its own parent folder containing every service it needs (`apps/<vertical>/<vertical>-instance/` for the frontend, `apps/<vertical>/<vertical>-api/` for the backend, plus any infra services such as `apps/<vertical>/<vertical>-postgres/`, `apps/<vertical>/<vertical>-redis/`, `apps/<vertical>/<vertical>-mqtt/`). Cross-vertical code lives in **`packages/*`** and aligns with the platform shell at **`apps/core-saas/core-saas-instance/`** — **not** a single customer-facing app where "vertical = only JSON."

**Layout rule (short):** `apps/<slug>/<slug>-<service>/`. The repeated `<slug>` prefix on the inner folder keeps the npm workspace name unambiguous. Workspaces glob: `apps/*/*` + `packages/*`.
...
| Area | Responsibility |
|------|----------------|
| `apps/core-saas/core-saas-instance/` | Platform shell — shared engine / entry patterns other instances align with |
| `apps/<vertical>/<vertical>-instance/` | Vertical-specific UI, routes, domain logic, branding hooks |
| `apps/<vertical>/<vertical>-api/` | Vertical-specific backend (Fastify/Express/Hono per `app.stack.json`) |
| `apps/<vertical>/<vertical>-{postgres,redis,mqtt,…}/` | Vertical-specific infra service configs co-located with the product (when the stack requires them) |
| `packages/*` | Cross-vertical libraries — **prefer extending here** over duplicating across instances |
| `configs/apps/<vertical>/<vertical>.json` | **Inputs** to spec generation and product metadata — not a substitute for instance code unless we add a true config runtime later |

For a narrative on **`apps/core-saas`** (why the folder exists while still thin, and how instances connect), see **[CORE-SAAS-PURPOSE.md](CORE-SAAS-PURPOSE.md)**.
...
- **Config's role:** **`configs/apps/<vertical>/<vertical>.json`** feeds **`generate-spec`** and the spec — it does **not** auto-generate runnable app code without Builder/Dev work.
...
- **New verticals** — add **`configs/apps/<vertical>/<vertical>.json`** and run **`npm run generate-spec -- <vertical>`** locally (or add a dedicated CI/workflow later if you want remote regeneration).
