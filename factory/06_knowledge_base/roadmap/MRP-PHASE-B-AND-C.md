...
This document explains **two optional upgrades** after **Phase A** (the deterministic planner: `factory/task-queue.json` fields, `npm run factory:next`, `computeParallelBatches` — see **[FACTORY-PROCESS.md](../process/FACTORY-PROCESS.md)**). Phase A is **planning only**. Phases B and C add a **single screen** (control tower) and optionally **automated execution**—still with human-centered controls (jidoka).
...
Reuse **`computeParallelBatches`** from `factory/factory_libs/planning/task-graph.ts`:

- Either **compile** the factory package into something the front end can import (monorepo shared package), or
- Expose a **tiny HTTP handler** that runs the same TypeScript on the server and returns `{ waves, waveCount, maxParallelism }` (same shape as a one-off script calling `computeParallelBatches`).
