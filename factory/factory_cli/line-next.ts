...
import { nextAgentPromptLine, planNext, planNextToJson, qualityAgentPromptLine } from "../01_production_planning/01_01_scheduling_orders/planner.js";
import { recordRun, repoRootFromHere } from "../factory_internal_ops/telemetry.js";

/** Next pullable task for the assembly line: dependency-ready, respects WIP cap and priority. Does not mutate the queue — pair with `mfg line queue` (remaining) and `mfg line done` (finished). */
...
async function main(): Promise<void> {
  ...
  const result = await recordRun(repoRoot, {
    ...
    command: `npm run mfg -- line next${asJson ? " -- --json" : ""}${queuePath ? ` -- --queue=${queuePath}` : ""}`,
  ...
  const t = result.task;
  console.log(`Next task for the assembly line (WIP ${result.wip.current}/${result.wip.cap}):`);
  console.log("");
  console.log(`  id:      ${t.id}`);
