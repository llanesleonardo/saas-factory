...
async function readActiveTimer(localDir: string): Promise<ActiveTimer | null> {
  const p = path.join(localDir, "run", "active-timer.json");
  try {
...
async function appendEvent(localDir: string, event: TimerEvent): Promise<void> {
  ...
  const logPath = path.join(localDir, "run", `time-tracking-${day}.jsonl`);
...
async function main(): Promise<void> {
  ...
  if (!cmd || cmd === "--help" || cmd === "-h") {
    console.log(
      [
        "Usage:",
        "  npm run mfg -- line time -- start --app <app> [--agent <role>] [--note <text>]",
        "  npm run mfg -- line time -- stop [--note <text>]",
        "  npm run mfg -- line time -- report [--day YYYY-MM-DD] [--app <app>]",
        "",
        "Notes:",
        "- Logs are stored locally under factory/telemetry/run/ (gitignored).",
