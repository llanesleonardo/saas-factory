...
function runValidate(kind: "pm" | "dev" | "quality", fixturePath: string): { exitCode: number; stderr: string } {
  const r = spawnSync("npm", ["run", "validate-agent-output", "--", kind, fixturePath], {
...
async function main(): Promise<void> {
  ...
  assert(files.length > 0, "No fixture files found under factory/fixtures/agent-output/");

  const failures: string[] = [];
  for (const f of files) {
    const kind = agentKindForFile(f);
    const expected = kindForFile(f);
    const { exitCode, stderr } = runValidate(kind, f);
