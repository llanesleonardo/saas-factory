import { assertQueueIntegrity, loadTaskQueue, normalizeTaskStatus, type FactoryTask } from "../../../factory_libs/planning/task-graph.js";
import registry from "../../../02_workforce/02_00_agents/agent-registry.json" with { type: "json" };
...
function assertOptionalRoutingLists(tasks: FactoryTask[]): void {
  const keys = ["materials", "workcenters", "services"] as const;
  for (const t of tasks) {
    for (const k of keys) {
      const v = (t as any)[k];
      if (v === undefined) continue;
      if (!Array.isArray(v)) {
        throw new Error(`Task ${t.id}: "${k}" must be an array of strings when present`);
      }
      for (let i = 0; i < v.length; i++) {
        const item = v[i];
        if (typeof item !== "string" || item.trim().length === 0) {
          throw new Error(`Task ${t.id}: "${k}[${i}]" must be a non-empty string`);
        }
      }
    }
  }
