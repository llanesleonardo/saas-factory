/**
 * Validates agent JSON payloads against factory/factory_schemas/*.schema.json (subset enforced in code).
 * Usage:
 *   npx tsx factory/03_assembly_lines/06-gates/validation/validate-agent-output.ts pm <file.json>
 *   … | npx tsx factory/03_assembly_lines/06-gates/validation/validate-agent-output.ts pm   # stdin JSON
 */
...
function validateQuality(data: unknown): void {
  ...
  if (!Array.isArray((data as any).errors)) msgs.push("errors must be array (empty on pass)");
  if (st === "fail" && Array.isArray((data as any).errors) && (data as any).errors.length === 0) {
    msgs.push('when status is "fail", errors[] should be non-empty');
  }
  if ((data as any).scope !== undefined) {
    const allowedScope = new Set(["gates", "harness", "harness_and_gates"]);
    if (typeof (data as any).scope !== "string" || !allowedScope.has((data as any).scope)) {
      msgs.push('scope must be one of: "gates" | "harness" | "harness_and_gates"');
    }
  }
  if ((data as any).summary !== undefined && typeof (data as any).summary !== "string") msgs.push("summary must be string");
  assertStringArray((data as any).checks_executed, "checks_executed", msgs);
  assertStringArray((data as any).acceptance_criteria_verified, "acceptance_criteria_verified", msgs);
  assertStringArray((data as any).commands_run, "commands_run", msgs);
  assertStringArray((data as any).screenshots, "screenshots", msgs);
  if ((data as any).coverage_summary !== undefined && typeof (data as any).coverage_summary !== "string") msgs.push("coverage_summary must be string");
  if ((data as any).final_verdict !== undefined && typeof (data as any).final_verdict !== "string") msgs.push("final_verdict must be string");

  const allowedLayer = new Set(["api", "ui", "db", "auth", "integration", "harness", "flake_suspected"]);
  if (Array.isArray((data as any).errors)) {
    (data as any).errors.forEach((e: unknown, i: number) => {
      const p = `errors[${i}]`;
      if (!isRecord(e)) {
      ...
      if ((e as any).layer !== undefined && (typeof (e as any).layer !== "string" || !allowedLayer.has((e as any).layer))) {
        msgs.push(`${p}.layer invalid`);
      }
      if ((e as any).file !== undefined && typeof (e as any).file !== "string") msgs.push(`${p}.file must be string`);
      if ((e as any).evidence !== undefined && typeof (e as any).evidence !== "string") msgs.push(`${p}.evidence must be string`);
      if ((e as any).how_to_repro !== undefined && typeof (e as any).how_to_repro !== "string") msgs.push(`${p}.how_to_repro must be string`);
      if ((e as any).acceptance_criterion !== undefined && typeof (e as any).acceptance_criterion !== "string") {
        msgs.push(`${p}.acceptance_criterion must be string`);
      }
    });
  }
  if ((data as any).issues_found !== undefined && !Array.isArray((data as any).issues_found)) {
    msgs.push("issues_found must be array when present");
  }
  if ((data as any).handoff_to !== undefined) {
    const allowedHandoff = new Set(["fix", "git", "dev", "tooling", "pm"]);
    if (typeof (data as any).handoff_to !== "string" || !allowedHandoff.has((data as any).handoff_to)) {
      msgs.push("handoff_to invalid");
    }
  }
  assertStringArray((data as any).known_issues_documented, "known_issues_documented", msgs);
  if (msgs.length) err(msgs);
}
