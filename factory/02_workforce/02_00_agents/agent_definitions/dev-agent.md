...
- Code + tests as needed; summary of edits; optional structured handoff JSON (**`factory/factory_schemas/dev-output.schema.json`**).
...
- **`factory/02_workforce/02_00_agents/context-packs/dev.json`** · **`factory/02_workforce/02_00_agents/agent-registry.json`** (`dev`)
...
**Env vars**, build scripts, local service URLs (DB, Redis, etc.), CI implications (**GitHub Actions**), where logs/metrics surface—enough to avoid “works on my machine” and to pair with **DevOps** on deploy deltas.
...
| **DX** | **Cursor** + Composer; **`tsx`** for TS CLIs; local **`.env`** / host services for parity with CI |
