...
import { isTaskDone, loadTaskQueue, orderTasks, type FactoryTask } from "../../factory_libs/planning/task-graph.js";
import { recordRun, repoRootFromHere } from "../../factory_internal_ops/telemetry.js";

export type { FactoryTask, TaskStatus } from "../../factory_libs/planning/task-graph.js";
export {
  loadTaskQueue,
  orderTasks,
...
export async function runOrchestrator(queuePath?: string): Promise<void> {
  ...
  const tasks = await recordRun(
    repoRoot,
    {
      ...
      command: `npm run mfg -- line orchestrate${queuePath ? ` -- --queue=${queuePath}` : ""}`,
      queue_path: queuePath,
      app: "factory/",
    },
    async () => orderTasks(queueTasks).filter((t: FactoryTask) => !isTaskDone(t)),
  );

  if (tasks.length === 0) {
    console.log("No tasks in factory/03_assembly_lines/03-registry/registry/task-queue.json. Paste PM JSON output there first.");
    return;
  }
