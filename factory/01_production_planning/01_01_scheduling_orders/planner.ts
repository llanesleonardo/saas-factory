...
export function planNext(tasks: FactoryTask[], wipCap: number): PlanNextResult {
  ...
  if (tasks.length === 0) {
    return { kind: "empty", message: "No tasks in factory/03_assembly_lines/03-registry/registry/task-queue.json." };
  }
...
export function planNextToJson(result: PlanNextResult): Record<string, unknown> {
  if (result.kind === "suggest") {
    ...
    const includeQuality =
      primaryRole === "dev" || primaryRole === "tooling" || primaryRole === "fix" || primaryRole === "devops";
    return {
      kind: result.kind,
      task: result.task,
      wip: result.wip,
