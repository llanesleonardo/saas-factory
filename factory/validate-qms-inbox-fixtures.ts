...
function runValidate(inboxDir: string): { exitCode: number; stderr: string } {
  const r = spawnSync("npm", ["run", "validate-qms-inbox", "--", `--dir=${inboxDir}`], {
...
async function main(): Promise<void> {
  ...
  assert(caseDirs.length > 0, "No fixture case directories found under factory/fixtures/qms-inbox/cases/");

  const failures: string[] = [];
  for (const caseDir of caseDirs) {
    const expected = kindForCaseDir(caseDir);
    const { exitCode, stderr } = runValidate(caseDir);
